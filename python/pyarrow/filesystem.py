...
class GCSFSWrapper(DaskFileSystem):
    ...
    @implements(FileSystem.isdir)
    def isdir(self, path):
        path = _gcs_norm_path(_stringify_path(path))
        try:
            contents = self.fs.ls(path)
            if len(contents) == 1 and contents[0] == path:
                return False
...
def _gcs_strip_protocol(path):
    if isinstance(path, list):
        return [_gcs_strip_protocol(p) for p in path]
    path = _stringify_path(path)
...
def _gcs_split_path(path):
    path = _gcs_strip_protocol(path).lstrip("/")
    if "/" not in path:
        return path, ""
    else:
        return path.split("/", 1)
